...
String AudioFile = "a6.mp3";
String HapticFile = "h6.wav";
...
#define THRESHOLD 800          // Threshold for analog trigger - 600 original
...
void loop() {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        if (currentState == HIGH && prevState[i] == LOW) {
                ...
                dfPlayer1.setVol(25);
                dfPlayer1.playSpecFile("/a6.mp3");
                if (useSecondPlayer) {
                    dfPlayer2.setVol(25);
                    dfPlayer2.playSpecFile("/h6.wav");
                }
                lastPlayTime = millis();
