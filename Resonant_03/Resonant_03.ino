...
String AudioFile = "a3.mp3";
String HapticFile = "h3.wav";
...
#define THRESHOLD 200          // Threshold for analog trigger
...
void loop() {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        if (currentState == HIGH && prevState[i] == LOW) {
                ...
                dfPlayer1.setVol(25);
                dfPlayer1.playSpecFile("/a3.mp3");
                if (useSecondPlayer) {
                    dfPlayer2.setVol(25);
                    dfPlayer2.playSpecFile("/h3.wav");
                }
                lastPlayTime = millis();
